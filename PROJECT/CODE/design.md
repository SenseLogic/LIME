...
All Cursor AI agents must generate code that **visually matches the existing
TypeScript files in `CODE/services/`**. Use those files (`bunny_service.ts`,
`contact_service.ts`, `file_service.ts`, `resend_service.ts`,
`stripe_service.ts`, `supabase_service.ts`) as the reference style.

### **Language & File Types**

- **TypeScript-first**
  - All newly generated application code must be **TypeScript**.
  - Use `.ts` for non-JSX modules and `.tsx` for JSX/Preact components.
  - Do not introduce new `.js` or `.jsx` files under `PROJECT/CODE/` unless
    there is a specific compatibility reason.

### **Formatting & Linting**

- **Formatter is the source of truth**
  - Code formatting is enforced by `deno fmt`. Do not try to preserve manual
    indentation/brace rules that conflict with the formatter.
  - Linting and type-checking are enforced by `deno lint` and `deno check`.

### **Indentation, Braces & General Layout**

- **Indentation**
  - Use **4 spaces per indentation level** in all `.js` and `.jsx` files (no
    tabs).
  - Nested blocks (classes, methods, `if`, loops, `try/catch`, chained calls)
    all increase indentation by 4 spaces.

- **Braces**
  - Opening braces are on their **own line**, directly under the construct they
    belong to:
  ```javascript
  ...
  ```
  - Closing braces are also on their **own line**, aligned vertically with the
    start of the construct.
  - This applies to `class`, `function`, `if`, `else`, `for`, `while`, `try`,
    `catch`, `finally`, and multi‑line object literals.
...
- **Function, method, and constructor declarations**
  - Follow the pattern used in `services`:
    ...
    - Closing parenthesis on its own line, aligned with the parameter
      indentation.
...
- **Multiline function calls**
  - For calls that span multiple lines, follow the style used in the services:
    ...
    - Each argument or configuration object starts on its **own line**, indented
      once (or twice) to align visually, consistent with existing services.
    - Closing parenthesis and semicolon on their own line, vertically aligned
      with the start of the call or with the surrounding style.
...
- **Chained method calls**
  - For chains (e.g. Supabase query builder), put each `.method` on its **own
    line**, indented one level deeper than the base:
  ```javascript
...
- **Parentheses in calls and conditions**
  - In function calls and conditionals, use a space after `(` and before `)`
    when there is content:
    - `if ( error !== null )`, `logError( error )`, `data[ 0 ]`.
  - When `()` or `[]` are empty, keep them together: `()`, `[]`.
  - For multiline parameter lists (as in declarations), follow the existing
    pattern in the services: opening and closing parentheses on their own lines,
    parameters on separate lines without extra padding around them.

- **Brackets**
  - When indexing arrays, use **one space** after `[` and before `]`, as in
    `data[ 0 ]`.
  - When passing arrays as arguments, match existing style, e.g.
...
- **Assignments split over multiple lines**
  - When destructuring or assigning complex expressions, follow the existing
    pattern:
    - `let { data, error } =` on one line.
    - The awaited call or expression starts on the next line, indented by 4
      spaces.
  ```javascript
  let { data, error } = await supabaseService.getClient()
    .from("CONTACT")
...
- **`let` vs `const`**
  - Match the services: prefer `let` for most variables, even when technically
    not reassigned, except for obvious fixed constants (e.g. module‑level
    configuration).
...
- Each parameter must be declared on its **own line**, following the style of
  the `services` folder.
...
- **No Tailwind CSS**
  ...
  - Only use **semantic CSS** defined in **global/shared SCSS files** inside the
    **styles** folder, which will all be imported from **main.scss**.
  - Each JSX component/island must define its specific styles inside a global
    SCSS file using the same file stem (`components/HomePage.jsx` =>
    `styles/HomePage.scss`).
- **Component‑scoped class naming**
  - CSS classes specific to a component or island must be prefixed with the
    component name in **slug‑case**.
...
Don't add any legacy data compatibility code to the codebase when some data
structures change. As this project is still not deployed in production, the code
and data must remain perfecly in sync.

### **Application to Generated Code**

- All **new or modified JavaScript code** must:
  - Match the indentation, brace placement, and chaining style demonstrated in
    `CODE/services/*.js`.
  - Respect the spacing rules for calls, conditions, operators, arrays, and
    brackets.
  - Follow the naming patterns for arrays and maps (e.g. `contactArray`,
    `contactByIdMap`, `recipientArray`).
...
These standards exist to keep the codebase **readable, self‑documenting, and
stylistically consistent with the existing services**. Any automated refactor or
new feature implementation must ensure these rules are applied.
