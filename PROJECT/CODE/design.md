...
-   **No Tailwind CSS**
    ...
    -   Only use **semantic CSS** defined in **global/shared SCSS files** inside the **styles** folder, which will all be imported from **main.scss**.
    -   Each JSX component/island must define its specific styles inside a global SCSS file using the same file stem (`components/HomePage.jsx` => `styles/HomePage.scss`).
